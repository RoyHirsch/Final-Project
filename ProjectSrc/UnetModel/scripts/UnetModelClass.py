		...
		with self.graph.as_default():
			...
			for l in range(1, self.layers + 2):
				if l == 1:
					with tf.name_scope('convolution_Down_{}'.format(l)):
						...
						self.weights_dict['WD2_{}'.format(l)] = weight_variable(
							[self.kernel_size, self.kernel_size, self.depth, self.depth])
					...
					with tf.name_scope('convolution_Down_{}'.format(l)):
						self.weights_dict['WD1_{}'.format(l)] = weight_variable([self.kernel_size, self.kernel_size,
						                                                         int(self.depth * self.ndepth / 2),
						                                                         self.depth * self.ndepth])
			...
			for l in self.layersTodisplay:
				for k in range(self.convd_dict['convd1_{}'.format(l)].shape[3] - 1):
					tempMerge1 = tf.summary.image('convlayer1_{}_{}'.format(l, k),
					                              self.convd_dict['convd1_{}'.format(l)][1:2, :, :, k:k + 1])
					displaylist.append(tempMerge1)
				for k in range(self.convd_dict['convd1_{}'.format(l)].shape[3]):
					tempMerge2 = tf.summary.image('convlayer2_{}_{}'.format(l, k),
					                              self.convd_dict['convd2_{}'.format(l)][1:2, :, :, k:k + 1])
			...
			with tf.name_scope('Finel_Layer'):
				...
				return tf.nn.conv2d(self.convu_dict['convu2_{}'.format(l)], Wfc, strides=[1, 1, 1, 1],
				                    padding='SAME') + bfc
		...
		with self.graph.as_default():
			if self.costStr == "softmax":
				...
				if 'weightedSum' in self.argsDict.keys() and self.argsDict['weightedSum']:
					loss = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(targets=self.Y, logits=self.logits,
					                                                               pos_weight=self.argsDict[
						                                                               'weightVal']))
				else:
		...
		with self.graph.as_default():
			...
			with tf.control_dependencies(update_ops):
				...
				if self.optStr == 'adam':
					...
					optimizer = tf.train.MomentumOptimizer(learning_rate=learningRate, momentum=momentum).minimize(
						self.loss)
				else:
