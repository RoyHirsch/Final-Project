...
# load data
dataPipe = DataPipline(numTrain=10, numVal=4, numTest=4, modalityList=[1,3],permotate=False,
...
# create net model
unetModel = UnetModelClass(layers=2, num_channels=len(dataPipe.modalityList), num_labels=1, image_size=240, kernel_size=3, depth=32,
                        pool_size=2, costStr='sigmoid', optStr='adam',weightedsum='True',weightval=10, argsDict={})

# train
trainModel = Trainer(net=unetModel, batchSize=6, argsDict={})
trainModel.train(dataPipe=dataPipe, logPath=LOG_DIR, outPath='', numSteps=600, restore=True, restorePath='/variables/unet_110318.ckpt')
