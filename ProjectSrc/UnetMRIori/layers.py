...
#####################################################3D#############################################################
def weight_variable3d(shape, stddev=0.1):
    """
    weight_variable = [filter_depth, filter_height, filter_width, in_channels, out_channels]
    """
    initial = tf.truncated_normal(shape, stddev=stddev)
    return tf.Variable(initial)
def weight_variable_devonc3d(shape, stddev=0.1):
    """
    weight_variable = [depth,height, width, output_channels, in_channels]
    """
    return tf.Variable(tf.truncated_normal(shape, stddev=stddev))

def bias_variable3d(shape):
    initial = tf.constant(0.1, shape=shape)
    return tf.Variable(initial)
...
def deconv3d(x, W, b, stride):
    xShape = tf.shape(x)
    output_shape = tf.stack([xShape[0], xShape[1]*2, xShape[2]*2,xShape[3]*2, xShape[4]//2])
    dconv = tf.nn.conv3d_transpose(x, W, output_shape, strides=[1, stride ,stride, stride, 1], padding='SAME')
    return tf.nn.relu(dconv + b)
